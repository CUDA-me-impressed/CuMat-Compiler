...
llvm::Value* Utils::SymbolTable::getValue(const std::string& symbolName) {
    if(inScope(symbolName)) {
        ...
        int curScope = locations.at(std::distance(locations.begin(), std::max_element(locations.begin(), locations.end())));
...
void Utils::SymbolTable::setValue(const std::string& symbolName, llvm::Value* storeVal) {
    ...
    if(this->variableLocations[symbolName].empty() || this->variableLocations[symbolName].back() != this->data.size()){
