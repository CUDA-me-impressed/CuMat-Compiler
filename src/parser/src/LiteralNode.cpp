...
template <>
llvm::Value* AST::LiteralNode<int>::codeGen(llvm::Module* module, llvm::IRBuilder<>* Builder, llvm::Function* fp) {
    if (type->primType != Typing::PRIMITIVE::INT) return nullptr;
    auto ty = static_cast<llvm::Type*>(llvm::Type::getInt64Ty(module->getContext()));
    return llvm::ConstantInt::get(ty, llvm::APInt(64, value, true));
}

template <>
llvm::Value* AST::LiteralNode<float>::codeGen(llvm::Module* module, llvm::IRBuilder<>* Builder, llvm::Function* fp) {
    if (type->primType != Typing::PRIMITIVE::FLOAT) return nullptr;
    auto ty = llvm::Type::getFloatTy(module->getContext());
    return llvm::ConstantFP::get(ty, llvm::APFloat(value));
}

template <>
llvm::Value* AST::LiteralNode<std::string>::codeGen(llvm::Module* module, llvm::IRBuilder<>* Builder,
                                                    llvm::Function* fp) {
    llvm::Type* ty;
    return nullptr;
}
