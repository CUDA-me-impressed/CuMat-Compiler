...
llvm::AllocaInst* Utils::createMatrix(Utils::IRContext* context, const Typing::Type& type) {
    ...
    // initial
    llvm::ArrayType* matRawType = llvm::ArrayType::get(ty, matLength + matType.rank + 1);
    ...
    for (int i = 0; i <= matType.rank; i++) {
        auto index = llvm::ConstantInt::get(context->module->getContext(), llvm::APInt(64, i));
        auto ptr = llvm::GetElementPtrInst::Create(matRawType, matAlloc, {zero, index}, "",
                                                   context->Builder->GetInsertBlock());
        llvm::Value* val = (i == 0) ? rank
                                    : llvm::ConstantInt::get(context->module->getContext(),
