...
antlrcpp::Any CuMatVisitor::visitProgram(CuMatParser::ProgramContext *ctx) {
    auto n = std::make_unique<AST::Node>(ctx->getText());
    auto children =
        this->visitChildren(ctx).as<std::vector<std::unique_ptr<AST::Node>>>();
...
antlrcpp::Any CuMatVisitor::aggregateResult(antlrcpp::Any aggregate,
    ...
    if (aggregate.isNull()) {
        std::vector<std::unique_ptr<AST::Node>> container;
        return container;
    }

    aggregate.as<std::vector<std::unique_ptr<AST::Node>>>().push_back(
        nextResult.as<std::unique_ptr<AST::Node>>());
    return aggregate;
}
