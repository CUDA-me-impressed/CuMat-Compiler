...
void AST::MatrixASTNode::codegen(){
    ...
    switch (this->type->primType) {
        case Typing::PRIMITIVE::INT: {
            type = llvm::IntegerType::getInt64Ty(LLVMUtils::module->getContext());
            break;
        } case Typing::PRIMITIVE::FLOAT: {
            type = llvm::IntegerType::getFloatTy(LLVMUtils::module->getContext());
            break;
        } case Typing::PRIMITIVE::BOOL: {
            type = llvm::IntegerType::getInt1Ty(LLVMUtils::module->getContext());
            break;
        }
        default: {
