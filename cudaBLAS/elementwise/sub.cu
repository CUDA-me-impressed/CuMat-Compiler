...
// Device function
__global__ void CuMatSubMatrixDKernel(double* A, double* B, double * res, long i, long j){
...
extern "C" void CuMatSubMatrixD(double * matA, double * matB, double * matRes, long i, long j){
    ...
    // Call the kernel
    CuMatSubMatrixDKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_Res, i, j);
...
// Device function
__global__ void CuMatSubMatrixIKernel(long* A, long* B, long * res, long i, long j){
    long N = i * j; // Treat matrix add as vector add (same thing for equal sizes)
    long index = blockDim.x * blockIdx.x + threadIdx.x;
    if(index < N){
        res[index] = A[index] - B[index];
...
extern "C" void CuMatSubMatrixI(long * matA, long * matB, long * matRes, long i, long j){
    ...
    // Call the kernel
    CuMatSubMatrixIKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_Res, i, j);
